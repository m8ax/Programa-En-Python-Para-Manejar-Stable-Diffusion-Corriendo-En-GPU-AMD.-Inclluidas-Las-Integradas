...
def run(
    ...
    if bench == 1:
        ...
        if tmpgpu > tmpcpu:
            cv2.imshow(
                ...
                " Es Un " + str(round(pcen, 2)) + "% Mas Rapida",
                Hori,
            )
        else:
            cv2.imshow(
            ...
            if tmpgpu > tmpcpu:
                ...
                if hablo == 1:
                    dime(
                        ...
                        " U, Es Un " + str(round(pcen, 2)) + "% Más Rápida..."
                    )
            else:
                resu = tmpcpu / tmpgpu
                pcen = (tmpcpu * 100) / tmpgpu
        ...
        with open("BenchMark - " + creadire + ".TxT", "w", encoding="utf-8") as file:
            ...
            if tmpgpu > tmpcpu:
                ...
                resficpu = (
                    ...
                    " Es Un " + str(round(pcen, 2)) + "% Más Rápida."
                )
                file.write(resficpu)
            else:
                resu = tmpcpu / tmpgpu
    ...
    while num_images < cuan:
            ...
            if num_images == 0:
                ...
                if word != "":
                    frase4 = (
                        ...
                        " Lexica Art, Que En Este Caso Es " + str(word) + ".\n"
                    )
                if filepro != "":
                    frase4 = ""
                    frase5 = (
                ...
                frase2a = (
                    ...
                    " Imágenes, " + str(modocale[sched]) + ".\n"
                )
                if calend != "":
                    frase19 = (
                        "En La Parte Superior Izquierda De La Imágen Generada, Se Incluirá El Calendario Anual, Del Año "
                        + calend
                        + ".\n"
                    )
                consolacomentada = (
        ...
        if calend != "":
            ...
            calendario = calendar.LocaleTextCalendar(locale="es")
            caleninsertar = calendario.formatyear(int(calend), 1, 1, 1, 3)
            if wwidth == 512:
                font = ImageFont.truetype("consola.ttf", 12)
            if wwidth < 512:
                font = ImageFont.truetype("consola.ttf", 4)
            if wwidth > 512:
                font = ImageFont.truetype("consola.ttf", 25)
            imagecalend = Image.open("Fin-2.PnG")
            draw = ImageDraw.Draw(imagecalend)
            draw.text(
                (10, 10),
                caleninsertar,
                font=font,
        ...
        with open(fichero, "a", encoding="utf-8") as file:
            ...
            if calend != "":
                file.write(
                    "\n\nCalendario Del Año "
                    + str(calend)
                    + ", Añadido A La Parte Superior Izquierda, De La Imágen."
                )
            if blane == 1:
    ...
    if fps > 0:
        ...
        ttfps = (
            "\n((( - Codificación De Video De Imágenes Generadas, Realizada A - ))) -"
            " ((( - " + str(round(nn / (tfine - tcomi), 3)) + " Imágenes/Seg. - )))\n"
        )
        print(
            "\nTamaño De Todas Las Imágenes Generadas - ((("
            f" {round((tamano+tamano3),3)} MB. )))"
    ...
    calendario = calendar.LocaleTextCalendar(locale="es")
    year = date.today()
    calendario_mes = calendario.formatmonth(year.year, year.month)
